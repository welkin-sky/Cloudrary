...
class Process {
 public:
  ...
  void ls() {
    ...
    for (const auto &i : base.at(now)) {
      cout << i.getName() << '\t' << i.getTitle() << '\t' << i.getAuthor()
           << '\t' << i.getTextPath() << '\t' << i.getDescription()
           << std::endl;
    }
  }
  ...
  void sortby(const string &str) {
    BookList b;
    static const char *method[5] = {"name", "title", "path", "author",
                                    "description"};
    int i = 0;
    for (; i < 5; i++) {
      if (str == method[i]) {
    ...
    if (i == 5)
      cout << "Wrong method! Available method: name, title, path, author, "
              "description.\n";
    else
      ls();
  }
  void download() {
    ...
    fmt::print(fg(fmt::color::green), "URL: ");
    cin >> url;
    fmt::print(fg(fmt::color::green), "Path: ");
    cin >> outputFilePath;
...
void cli(Base &base, std::ofstream &out) {
  static const string usage =
      ...
      "/ addlib / removebook / removelib / download / sortby [method] / exit\n";
  Process p(base);
