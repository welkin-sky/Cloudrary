...
class BookList {  // 无名的书单，用于临时对象（如筛选结果）
 ...
 public:
  ...
  [[nodiscard]] INT find(const Book &book) const {
    return std::distance(begin(), std::find(cbegin(), cend(), book));
  }

  BookList &sortBy(const function<bool(const Book &, const Book &)> &comp =
                       Book::CompareName) {
  ...
  BookList sublist(INT first, INT last) {
    return {vector<Book>(_books.begin() + first, _books.begin() + last)};
  }
